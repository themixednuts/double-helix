...
| `bufferline` | Renders a line at the top of the editor displaying open buffers. Can be a string (`always`, `never`, or `multiple`) or an `[editor.bufferline]` table. | `"never"` |
| `bufferline.render-mode` | Bufferline visibility when using `[editor.bufferline]`. Can be `always`, `never`, or `multiple` (only shown if more than one buffer is in use). | `"never"` |
