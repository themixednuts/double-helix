...
| `assistant_panel` | Toggle assistant panel | normal: `` <space>Aa ``, `` <space>Af ``, select: `` <space>Aa ``, `` <space>Af `` |
| `assistant_close_panel` | Close assistant panel | normal: `` <space>Aq ``, select: `` <space>Aq `` |
| `assistant_focus_input` | Focus assistant panel and activate input | normal: `` <space>Ai ``, select: `` <space>Ai `` |
| `assistant_focus_entries` | Focus assistant panel entry list | normal: `` <space>Am ``, select: `` <space>Am `` |
| `assistant_open_entry_scratch` | Open selected assistant entry details in current view | normal: `` <space>Ad ``, select: `` <space>Ad `` |
| `assistant_cycle_thinking` | Cycle assistant thinking options | normal: `` <space>A<C-t> ``, select: `` <space>A<C-t> `` |
| `assistant_cycle_model` | Cycle assistant model options | normal: `` <space>A<C-m> ``, select: `` <space>A<C-m> `` |
| `assistant_cycle_mode` | Cycle assistant mode options | normal: `` <space>A<S-tab> ``, select: `` <space>A<S-tab> `` |
| `assistant_toggle_follow` | Toggle following for the active assistant thread | normal: `` <space>At ``, select: `` <space>At `` |
...
| `textobject_word` | Select inside word |  |
| `textobject_long_word` | Select inside WORD |  |
| `textobject_paragraph` | Select inside paragraph |  |
| `textobject_closest_surrounding_pair` | Select inside closest surrounding pair (tree-sitter) |  |
| `textobject_function` | Select function (tree-sitter) |  |
| `textobject_class` | Select class/type (tree-sitter) |  |
| `textobject_parameter` | Select inside argument/parameter (tree-sitter) |  |
| `textobject_comment` | Select inside comment (tree-sitter) |  |
| `textobject_test` | Select inside test (tree-sitter) |  |
| `textobject_entry` | Select inside data structure entry (tree-sitter) |  |
...
| `textobject_change` | Select inside VCS change |  |
...
| `select_textobject_inside_surrounding_pair` | Select inside surrounding pair |  |
| `select_textobject_around_surrounding_pair` | Select around surrounding pair |  |
| `select_textobject_inside_prev_pair` | Select inside previous pair |  |
| `select_textobject_inside_next_pair` | Select inside next pair |  |
