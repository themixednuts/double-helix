...
| Assistant messages | `j`/`k`, `Down`/`Up`, `Ctrl-n`/`Ctrl-p`, `PageDown`/`PageUp`, `Ctrl-d`/`Ctrl-u`, `G`/`gg`, `End`/`Home`, mouse wheel | `Enter` opens details or jumps to a subagent session | `a`/`A` accept review, `x`/`X` reject review, `r` retry failed/canceled prompt, `R` review mode, `t` follow | `Esc` returns to input or interrupts a running agent; `Ctrl-c` interrupts | `Tab` | `y` | N/A |
