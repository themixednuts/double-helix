...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Assistant history | Persisted assistant threads: entries, turns, plan/draft/context/follow-run state, unread flag, mode/config/profile, feedback, review mode, scope, view, terminals. See `helix-view/src/assistant/history/local.rs:14`, `:23`, `:27`, `:83`, `:136`. | File-per-thread pretty JSON under `cache_dir()/assistant/history/{id}.json`. | Debounced saves, flush on close/shutdown. Listing scans every `*.json`, decodes all records, sorts by id, and filters by scope. | Multiple thread saves can happen; each file write now uses shared atomic writer. No cross-record transaction. | Strong. History browser, scope filtering, ratings/notes/feedback, future search and retention all want indexed records. | Grows with assistant usage; records may include terminal/session context and can become large. | Per-file atomic rename via `assistant/layout.rs:89`, but no database-level consistency or indexes. Corrupt records are skipped. | SQLITE, first wave. |
| Assistant layout | Persisted layout state per scope. See `helix-view/src/assistant/layout.rs:20`, `:54`, `:89`, `:145`. | Single aggregate JSON file at `cache_dir()/assistant/layout.json`. | Read on load, read-modify-write on layout save, flush via assistant persistence. | A static async mutex serializes layout saves in-process. | Low by itself; keyed by scope. | Small. | Recently made atomic with temp-file write, `sync_all`, and rename. No parent-dir fsync; no interprocess lock. | SQLITE if a store exists; otherwise FILE is acceptable. Migrating with history avoids repeating hand-written atomic RMW logic. |
| Assistant permissions | Agent/tool permission rules. See `helix-view/src/assistant/permission.rs:249`, `:261`, `:275`. | Single TOML file at `cache_dir()/assistant/permissions.toml`. | Read at startup or permission manager creation; written when rules change/reset. | Direct `std::fs::write`; no atomic write and no lock. | Moderate. Matching scans rules in memory, but rules are naturally tabular by agent/tool/choice. | Small. | Not atomic; decode failures fall back to default rules. | SQLITE or FILE. Prefer SQLITE if introducing a unified assistant store; TOML remains defensible if human editing is intended. |
...
| Package receipts | Installed package metadata: name, kind, version, source, archive hash, bins/shims, file list, installed_at, native-manager info. See `helix-pkg/src/store.rs:52`, `:76`, `:93`, `:205`, `:231`. | File-per-package TOML under `data_dir()/double-helix/pkg/receipts/{kind}-{name}.toml`. | Written on install/upgrade, scanned all for list/status, read individually by loader compatibility paths. | Per-package advisory lock exists for install staging; receipt writes are direct file writes. | Moderate. Installed package listing, lookup by kind/name, upgrade queries, stale receipt cleanup. | Grows with installed packages; usually small. | Direct `fs::write`, no atomic rename. Decode errors surface to callers. | SQLITE, second wave. Good generated-state candidate, but keep compatibility read/import path. |
...
| Plugin RPC codec | Plugin protocol messages. See `helix-plugin/src/contract/codec.rs:1`, `:20`. | MessagePack via `rmp-serde`, but only over RPC/wire. | Per plugin message. | Runtime protocol. | Not on disk. | N/A. | N/A. | FILE/none. Not persistent storage. |
| File-picker frecency | Path hash to timestamp deque for file open/access scoring. See `helix-term/src/fff.rs:369`, `vendor/fff-search/src/frecency.rs:25`, `:60`, `:132`, `:344`. | LMDB via `heed`, under `cache_dir()/fff/{workspace_hash}/frecency` as `data.mdb`/`lock.mdb`, values bincode-serialized. | Updated on file open/access, read while ranking files, occasional purge/GC. | LMDB handles transactions and locks; Helix passes `use_unsafe_no_lock = false`. | Strong. Ranking scans/scores many candidate files. | Grows with visited files per workspace. | Transactional LMDB. GC compacts by rebuilding/deleting LMDB files after collecting entries. | SQLITE, first or second wave. Already database-backed, but a unified SQLite store would reduce storage diversity. |
| File-picker query tracker | Query-to-opened-file associations and bounded file/grep query histories. See `helix-term/src/fff.rs:385`, `vendor/fff-search/src/query_tracker.rs:30`, `:69`, `:137`, `:190`, `:292`. | LMDB via `heed`, under `cache_dir()/fff/{workspace_hash}/queries`; bincode values. | Updated when a query completes/open succeeds; queried for query history and ranking hints. | LMDB transactional. | Strong. Query history and query-file associations are indexed state. | Bounded query history, association table grows with usage. | Transactional LMDB. | SQLITE, first or second wave. |
