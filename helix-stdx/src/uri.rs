...
impl Url {
    ...
    /// The authority (host) between `//` and the path, if the URI has one: empty for
    /// `file:///path`, `server` for `file://server/share`.
    pub fn authority(&self) -> Option<&str> {
        let after_scheme = &self.0[self.0.find(':')? + 1..];
        let rest = after_scheme.strip_prefix("//")?;
        Some(&rest[..rest.find(['/', '?', '#']).unwrap_or(rest.len())])
    }

    /// Build a `file://` URI from a path already in URL form (`/home/user/a.rs`,
    /// `/C:/dir/a.rs`), whatever the platform: for paths on another machine.
    pub fn from_url_path(path: &str) -> Url {
        let mut serialization = String::from("file://");
        if !path.starts_with('/') {
            serialization.push('/');
        }
        serialization.extend(percent_encode(path.as_bytes(), PATH));
        Url(serialization)
    }
...
mod tests {
    ...
    fn windows_file_path_round_trip() {
        for path in [
            r"C:\Users\user\main.rs",
            r"C:\tmp\a b.txt",
            r"C:\app\[slug]\page.tsx",
            r"D:\café\c#\Program.cs",
        ] {
            let url = Url::from_file_path(path).unwrap();
            assert_eq!(url.to_file_path().unwrap(), PathBuf::from(path), "{}", url);
        }
        let url = Url::from_file_path(r"C:\app\[slug]\a b.ts").unwrap();
        assert_eq!(url.as_str(), "file:///C:/app/%5Bslug%5D/a%20b.ts");
        // Servers that encode the drive colon (`c%3A`) still map back to the path.
        let url = Url::parse("file:///c%3A/app/main.rs").unwrap();
        assert_eq!(
            url.to_file_path().unwrap(),
            PathBuf::from(r"c:\app\main.rs")
        );
    }

    #[test]
    fn url_path_and_authority() {
        let url = Url::from_url_path("/C:/app/[slug]/a b.ts");
        assert_eq!(url.as_str(), "file:///C:/app/%5Bslug%5D/a%20b.ts");
        assert_eq!(url.authority(), Some(""));
        assert_eq!(
            Url::parse("file://server/share/a").unwrap().authority(),
            Some("server")
        );
        assert_eq!(Url::parse("untitled:foo").unwrap().authority(), None);
    }
