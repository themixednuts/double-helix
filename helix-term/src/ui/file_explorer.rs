...
impl VcsStatus {
    ...
    fn icon(self, icons: &Icons) -> &str {
        ...
        match self {
            Self::Added => configured_or(icons.vcs().added(), VCS_ADDED_ICON),
            Self::Modified => configured_or(icons.vcs().modified(), VCS_MODIFIED_ICON),
            Self::Deleted => configured_or(icons.vcs().removed(), VCS_DELETED_ICON),
            Self::Renamed => configured_or(icons.vcs().renamed(), VCS_RENAMED_ICON),
            Self::Conflict => configured_or(icons.vcs().conflict(), VCS_CONFLICT_ICON),
        }
    }
...
impl VcsSnapshot {
    fn load(editor: &Editor, root: &Path) -> Self {
        if !editor.config().file_explorer.vcs {
            return Self::default();
        }

        match editor.diff_providers.changed_files(root) {
            Ok(changes) => Self::from_changes(root, changes),
            Err(err) => {
                log::debug!("failed to collect file explorer VCS status: {err:#?}");
                Self::default()
            }
        }
    }

    fn from_changes(root: &Path, changes: impl IntoIterator<Item = FileChange>) -> Self {
    ...
    fn status(&self, path: &Path) -> Option<VcsStatus> {
...
struct RowBuildContext<'a> {
    editor: &'a Editor,
    vcs: &'a VcsSnapshot,
    seen: &'a mut HashSet<PathBuf>,
    rows: &'a mut Vec<ExplorerRow>,
}
...
impl FileExplorerPanel {
    ...
    pub fn refresh(
        ...
        let vcs = VcsSnapshot::load(editor, &self.root);
        ...
        rows.push(ExplorerRow {
            ...
            vcs_status: vcs.status(&self.root),
        });
        if root_expanded {
            ...
            self.collect_rows(&self.root, 1, &[], &mut build)?;
        }
        self.rows = rows;
    ...
    fn collect_rows(
        ...
        let children = directory_children(root, build.editor)?;
        ...
        for (index, (path, is_dir)) in children.into_iter().enumerate() {
            ...
            build.rows.push(ExplorerRow {
                ...
                vcs_status: build.vcs.status(&path),
            });
            ...
            if build.seen.insert(canonical) {
                ...
                self.collect_rows(&path, depth + 1, &child_ancestors, build)?;
            }
        }
        Ok(())
    }
    ...
    fn row_spans(
        ...
        let mut spans = Vec::with_capacity(8 + row.ancestor_last.len());
        ...
        if row.depth > 0 {
            let connector = if row.is_last { TREE_LAST } else { TREE_MIDDLE };
...
impl Component for FileExplorerPanel {
    ...
    fn render(&mut self, area: Rect, surface: &mut Surface, cx: &RenderContext) {
            ...
            .take(list.height as usize)
        {
            ...
            let spans = self.row_spans(
                row,
                base_style,
                directory_style,
                guide_style,
                cx.editor.config().file_explorer.icons,
            );
            let status_width = row.vcs_status.map_or(0, |_| 2);
            draw_spans(
                surface,
                list.x,
                y,
                list.width.saturating_sub(status_width),
                &spans,
            );
            if let Some(status) = row.vcs_status {
                let icons = ICONS.load();
                let icon = status.icon(&icons);
                let icon_width = icon.chars().count().max(1) as u16;
                let x = list.right().saturating_sub(icon_width);
                ...
                let status_style = if is_selected {
                    selected.patch(status_style)
                } else {
                    status_style
                };
                surface.set_stringn(x, y, icon, icon_width as usize, status_style);
            }
        }
    }
...
mod tests {
    ...
    fn row_spans_include_fallback_icons() {
        ...
        assert!(spans
            .iter()
            .any(|span| span.content.contains(FALLBACK_FOLDER_OPEN_ICON)));
    }
