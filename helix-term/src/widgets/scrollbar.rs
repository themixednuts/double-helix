...
impl Scrollbar {
    ...
    pub fn symbol(mut self, s: &'static str) -> Self {
        self.thumb_symbol = s;
        self
    }
    ...
    pub fn track(mut self, symbol: &'static str, style: Style) -> Self {
    ...
    pub fn render(self, area: Rect, surface: &mut Surface) {
        ...
        let thumb_top = ((height - thumb_height) * self.offset)
            .checked_div(max_offset)
            .unwrap_or(0);

        for i in 0..height {
            let cell = &mut surface[(area.x, area.y + i as u16)];
            if i >= thumb_top && i < thumb_top + thumb_height {
                cell.set_symbol(self.thumb_symbol);
                cell.set_style(self.thumb_style);
            } else if let Some((track_symbol, track_style)) = self.track {
