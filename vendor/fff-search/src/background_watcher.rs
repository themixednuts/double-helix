    ...
    for debounced_event in &events {
        ...
        if debounced_event.event.need_rescan() {
            if debounced_event.event.paths.len() < 16 // this should be usually one event
                && debounced_event
                    ...
                    .all(|p| should_include_path(p, repo.as_ref(), base_path, scan_options))
            {
                break;
            }
        ...
        for path in &debounced_event.event.paths {
            ...
            if is_folder_removal {
                ...
                if should_include_path(path, repo.as_ref(), base_path, scan_options) {
...
fn should_include_path(
    ...
    match repo {
        ...
        Some(_) => true,
        None => {
            // When we have no git repo apply basic sanity filters to prevent
