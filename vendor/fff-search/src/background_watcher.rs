...
use crate::file_picker::{FFFMode, FilePicker, FilePickerScanOptions};
...
impl BackgroundWatcher {
    pub fn new(
        ...
        let owner_base_path = base_path.clone();
        let owner_scan_options = scan_options.clone();
    ...
    for debounced_event in &events {
        ...
        for path in &debounced_event.event.paths {
            ...
            if is_removal || !path.exists() {
                ...
                if should_include_path(path, &repo, base_path, scan_options) {
...
fn should_include_path(
    ...
    if scan_options.hidden && has_hidden_component(path, base_path) {
        return false;
    }

    if !scan_options.follow_links
    ...
    if !scan_options.ignore {
        return true;
    }

    match repo.as_ref() {
        Some(repo) if scan_options.git_ignore => repo.is_path_ignored(path) != Ok(true),
        _ => !is_non_code_directory(path),
    }
}
...
fn has_hidden_component(path: &Path, base_path: &Path) -> bool {
