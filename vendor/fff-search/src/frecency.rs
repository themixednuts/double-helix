...
impl DbHealthChecker for FrecencyTracker {
    fn get_health(&self) -> Result<crate::db_healthcheck::DbHealth> {
...
mod tests {
    ...
    fn test_frecency_calculation() {
        ...
        let recent_frequent =
            VecDeque::from([current_time, current_time - 86400, current_time - 172800]);
        let old_single = VecDeque::from([current_time - ten_days_seconds]);

        let recent_score = FrecencyTracker::calculate_access_score(
            &recent_frequent,
            current_time,
            FFFMode::Neovim,
        );
        let old_score =
            FrecencyTracker::calculate_access_score(&old_single, current_time, FFFMode::Neovim);

        assert!(
            recent_score > old_score,
            "Recent frequent access ({recent_score}) should score higher than old single access ({old_score})",
        );
    }

    #[test]
    fn test_ranking_order_matches_legacy_algorithm() {
        let now = 1_700_000_000;
        let recent_frequent = VecDeque::from([now - 20, now - 10, now]);
        let recent_single = VecDeque::from([now - 5]);
        let stale_frequent = VecDeque::from([now - 25 * 86400, now - 20 * 86400, now - 15 * 86400]);

        let mut scored = [
            (
                "recent_single",
                FrecencyTracker::calculate_access_score(&recent_single, now, FFFMode::Neovim),
            ),
            (
                "stale_frequent",
                FrecencyTracker::calculate_access_score(&stale_frequent, now, FFFMode::Neovim),
            ),
            (
                "recent_frequent",
                FrecencyTracker::calculate_access_score(&recent_frequent, now, FFFMode::Neovim),
            ),
        ];
        scored.sort_by(|a, b| b.1.cmp(&a.1).then_with(|| a.0.cmp(b.0)));

        assert_eq!(scored[0].0, "recent_frequent");
        assert_eq!(scored[1].0, "recent_single");
        assert_eq!(scored[2].0, "stale_frequent");
    }
