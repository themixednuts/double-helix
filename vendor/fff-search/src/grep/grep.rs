...
impl<'a> NeedleFinder<'a> {
    pub(super) fn new(needle: &'a [u8], case_insensitive: bool) -> Self {
        if case_insensitive {
...
pub(super) struct PlainTextMatcher<'a> {
    pub(super) finder: &'a NeedleFinder<'a>,
}
...
pub(super) struct PlainTextSink<'r> {
    pub(super) state: SinkState,
    pub(super) finder: &'r NeedleFinder<'r>,
    pub(super) pattern_len: u32,
    pub(super) multiline_segment_len: Option<usize>,
}
