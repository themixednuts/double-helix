...
pub(crate) fn walk_collect_files(
    ...
    _symlink_scope_root: &Path,
    threads: usize,
    synced_files_count: &Arc<AtomicUsize>,
) -> crate::Result<WalkOutput> {
    // FFF_SCAN_OPTIONS_BLOCKER: the zlob backend only maps the scan options it
    // has flags for (hidden, follow_links). Helix builds the default `ripgrep`
    // backend, which honours the full `FilePickerScanOptions` set.
    let mut flags = WalkFlags::GITIGNORE;
    if scan_options.hidden {
