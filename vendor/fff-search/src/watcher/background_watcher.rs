...
use crate::scan_options::FilePickerScanOptions;
...
pub(crate) fn handle_debounced_events(
    ...
    // FFF_SCAN_OPTIONS_BLOCKER: the picker's scan options are grabbed alongside.
    let (walker_rules, scan_options) = shared_picker
        .read()
        .ok()
        .and_then(|g| {
            g.as_ref()
                .map(|p| (p.ignore_rules(), p.scan_options().clone()))
        })
        .unwrap_or_default();
    let filter = IgnoreFilter::new(base_path, walker_rules, repo.as_ref(), &scan_options);
...
fn index_new_directory(
    ...
    let (base_path, walker_rules, scan_options) = match shared_picker.read().ok().and_then(|g| {
        g.as_ref().map(|p| {
            (
                p.base_path().to_path_buf(),
                p.ignore_rules(),
                p.scan_options().clone(),
            )
        })
    }) {
        Some(triple) => triple,
    ...
    // FFF_SCAN_OPTIONS_BLOCKER: walk the new subtree with the picker's scan
    // options, keeping `max_depth` and symlink scope relative to the base.
    let Some(subtree_options) = scan_options.for_subtree(&base_path, dir) else {
        return Vec::new();
    };
...
struct IgnoreFilter<'a> {
    ...
    /// FFF_SCAN_OPTIONS_BLOCKER: the picker's scan options, applied before any
    /// ignore rules so watcher events agree with the walker.
    scan: &'a FilePickerScanOptions,
}
...
impl<'a> IgnoreFilter<'a> {
    ...
    fn is_ignored(&self, path: &Path) -> bool {
        // FFF_SCAN_OPTIONS_BLOCKER: structural policy (hidden, depth, symlinks)
        // first; `scan.ignore == false` disables ignore-rule filtering.
        if self.scan.excludes_path(path, self.base_path) {
            return true;
        }
        if !self.scan.ignore {
            return false;
        ...
        if let Some(rules) = self.rules.as_ref() {
            ...
            return self.scan.git_ignore && rules.is_ignored(relative);
        }
        match self.repo {
            Some(repo) => self.scan.git_ignore && repo.is_path_ignored(path) == Ok(true),
