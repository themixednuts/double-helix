...
use crate::file_picker::{FFFMode, FilePicker, FilePickerOptions, FilePickerScanOptions};
...
fn editing_an_indexed_file_is_never_mistaken_for_a_rename() {
    ...
    // Local fix: Windows rejects `set_modified` on a read-only handle.
...
impl Fixture {
    ...
    fn build(git: bool, frecency: bool) -> Self {
        ...
        let (shared_frecency, frecency_tmp) = if frecency {
            ...
            shared
                ...
                .init(FrecencyTracker::memory_only().expect("open frecency"))
    ...
    fn index(&self) {
        let mut picker = FilePicker::new(FilePickerOptions {
            ...
            watch: false,
            // FFF_SCAN_OPTIONS_BLOCKER: the vendored scan default hides dot
            // entries everywhere; these tests assume upstream's policy of
            // walking them inside git repos (`hidden(!is_git_repo)`).
            scan: FilePickerScanOptions {
                hidden: self.git_workdir.is_none(),
                ..Default::default()
            },
            ..Default::default()
        })
