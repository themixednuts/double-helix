...
fn input_matches_str<I: Input>(str: &str, range: Range<u32>, input: &mut I) -> bool {
    ...
    let start_in_chunk = range.start - cursor_offset;
    if start_in_chunk > cursor.chunk().len() {
        return false;
    }
    if range.end >= cursor_offset && range.end - cursor_offset <= cursor.chunk().len() {
        // hotpath
        return &cursor.chunk()[start_in_chunk..range.end - cursor_offset] == str;
    }
    let chunk_tail_len = cursor.chunk().len() - start_in_chunk;
    if cursor.chunk()[start_in_chunk..] != str[..chunk_tail_len] {
        return false;
    }
    str = &str[chunk_tail_len..];
    while cursor.advance() {
        if range.end < cursor.offset() {
            return false;
        }
        if str.len() <= cursor.chunk().len() {
            let end_in_chunk = range.end - cursor.offset();
            if end_in_chunk > cursor.chunk().len() {
                return false;
            }
            return &cursor.chunk()[..end_in_chunk] == str;
        }
...
mod tests {
    ...
    #[test]
    fn input_matches_str_rejects_out_of_bounds_ranges() {
